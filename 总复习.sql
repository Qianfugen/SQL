...
begin
    query_sal_by_empno(7369,out_sal);
...
is     
begin
       select * into emp_row from emp where ename=in_name;
end;
...
declare
       emp_row emp%rowtype;
begin
       query_all_by_name('KING',emp_row);
       dbms_output.put_line('编号:'||emp_row.empno||',姓名:'||emp_row.ename||',薪资:'||emp_row.sal);
end;

--根据用户名称模糊查询所有员工信息
create or replace procedure query_by_name_like(
       in_name in emp.ename%type,
       emp_cur out sys_refcursor
)
is
begin
       open emp_cur for select * from emp where ename like '%'||in_name||'%';
end;
...
--方式二
create or replace procedure query_all_emp2
is
       emp_cur sys_refcursor;
       emp_row emp%rowtype;
begin
...
/*
   方法  
       1. 必须要有返回值
       2. 可以直接在sql语句中调用     
       3. 过程可以返回任意数据类型以及任意多的结果,方法同样可以返回任意类型但是只能返回一个结果   
   使用存储过程与函数的原则:
...
--sql语句直接调用
select * from emp;
select add_sal_and_comm(7499) 总收入 from dual;

--使用过程调用
declare
       sal_and_comm emp.sal%type;
begin
       sal_and_comm := add_sal_and_comm(7499);
       dbms_output.put_line('总收入:'||sal_and_comm);
end;
